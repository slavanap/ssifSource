...
//
// This is the OLD DVDOpen() which is somewhat left-behind and broken.
//
#if 0
...
// This is the newest DVDOpen from 1.9.7
...
#ifndef WIN33 /* don't have fchdir, and getcwd( NULL, ... ) is strange */
    /* Resolve any symlinks and get the absolut dir name. */
    {
      char *new_path;
      char *current_path;
...
static dvd_file_t *DVDOpenFileUDF( dvd_reader_t *dvd, char *filename )
{
	...
	// This filesize will be completely incorrect for UDF2.50 style operations
	// but if I fix it here, it will break for old IFO/DVD files.
...
int DVDDiscID( dvd_reader_t *dvd, unsigned char *discid )
{
	...
	if( dvd_file != NULL ) {
	    ...
	    unsigned char *buffer_base = malloc( file_size + 2048 );
	    unsigned char *buffer = (unsigned char *)(((uintptr_t)buffer_base & ~((uintptr_t)2047)) + 2048);
...
#ifdef DEBUG
	fprintf(stderr,"Found '%s' at %d (size %"PRIu64")\n", subdir,
