...
/*
 * Change input methods for dvdread. Currently you can call it with NULL for
 * default dvdread methods (file io, dvdcss) or with the path to "unrar" and
 * a RAR archive for direct unrar streaming. Note it requires a special "unrar"
 * utility with the -sk <offset> option. Available from http://lundman.net/wiki
 *
 * unrar_path : path to unrar executable. "/usr/local/bin/unrar".
 * rarfile    : path to ISO file INSIDE rar archive. "/movie.iso".
 *
 * The call DVDOpen("/path/to/movie.rar");
 *
 * TODO: The methods should really be tied to a dvd_reader_t *, so they can
 * be mixed. Currently you can not even mix between plain and css calls.
 *
 */
...
typedef struct {
	...
	// To save most people from immediately calling stat(), and since we
	// don't really have a stat() API just yet.
	uint64_t       d_filesize;
} dvd_dirent_t;
